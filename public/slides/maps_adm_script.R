...
admitido <- procedencia.R[,33]
indi <- procedencia.R[,34]<-1




admitidos<-data.frame(depart_asp,codecity_asp,codept_asp, ciudad_asp,long_asp, lat_asp, sexo,admitido, mod_asp, tipo_asp, tipopeama_asp, indi)

#Omitir admitidos del extranjero

admitidos <- admitidos%>%filter(depart_asp!="DEPARTAMENTO EXTRANJERO")


#Omitir admitidos no admitidos

admitidos <- admitidos%>%filter(admitido!="No")

cantesp_asp <-admitidos %>% group_by(mod_asp, depart_asp, codept_asp)%>% summarise (n = n()) %>% filter(mod_asp=="Especial")
...
cantesp_asp_mun <-  admitidos %>% group_by(mod_asp, ciudad_asp, codecity_asp)%>% summarise (n = n()) %>% filter(mod_asp=="Especial")


#Extraer lista de códigos de los municipios con la respectiva cantidad de admitidos
...
colombia.R@data$CANT_ADM=cantesp_asp$n
...
tipo <- admitidos %>% group_by(depart_asp, codept_asp, tipo_asp)%>% summarise (n = n())
PEAMA <-  tipo %>% filter(tipo_asp=="PEAMA ")

tipopeama <- admitidos %>% group_by(depart_asp, codept_asp, tipopeama_asp)%>% summarise (n = n())
...
tipopeama <- admitidos %>% group_by(depart_asp, codept_asp, tipopeama_asp, lat_asp, long_asp, sexo)
orinoquia <- tipopeama %>%  filter(tipopeama_asp=="PEAMA - Orinoquía")
