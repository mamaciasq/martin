...
Microdatos <- bind_rows(P2008_1, P2008_2, P2009_1, P2009_2, P2010_1, P2010_2, P2011_1, 
  ...
  filter(ADMITIDO=="Sí") %>% filter(PEAMA=="PEAMA - Orinoquía")
...
DT12 <- Clases("ADM_SEDE_NOMBRE")
DT13 <- Clases("FACULTAD")
DT14 <- Clases("PROGRAMA")
DT15 <- Clases("AREAC_SNIES")
DT16 <- Clases("AREA_CINE")
...
Consolidado <- bind_rows(DT1, DT2, DT3, DT4, DT5, DT6, DT7, DT8, DT9, DT10, DT11, DT12, DT13, DT14, DT15, DT16, Total)
Puntaje <- Microdatos %>% mutate(Serie = paste(YEAR,SEMESTRE, sep = "-")) %>% select(Serie,PTOTAL)
Carreras <- Microdatos %>% group_by(YEAR, SEMESTRE, ADM_SEDE_NOMBRE, FACULTAD, PROGRAMA) %>% summarise(Total=n())
memory.size()
rm(P2008_1, P2008_2, P2009_1, P2009_2, P2010_1, P2010_2, P2011_1, 
   P2011_2, P2012_1, P2012_2, P2013_1, P2013_2, P2014_1, P2014_2, 
   P2015_1, P2015_2, P2016_1, P2016_2, P2017_1, P2017_2, P2018_1,
   DT1, DT2, DT3, DT4, DT5, DT6, DT7, DT8, DT9, DT10, DT11, DT12, 
   DT13, DT14, DT15,DT16, Microdatos,tipovar,importar,Clases, Total);
memory.size(); gc(); memory.size()
