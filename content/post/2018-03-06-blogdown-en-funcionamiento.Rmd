...
```
# por ejemplo, cree un sitio nuevo con el tema academic-theme blogdown::new_site(theme = "gcushen/hugo-academic", theme_example = TRUE)
```
...
Por último, su [encabezado de YAMLdebe lucir similar a esto](https://bookdown.org/yihui/blogdown/output-format.html#output-format); note que algunos mas no todas las prestaciones de `rmarkdown::html_document` [son soportadas en `blogdown`](https://bookdown.org/yihui/blogdown/output-format.html#fn15):
