...
Hay otros tres campos que le permiten expresar dependencias más especializadas:
  ...
  * `Depends`: antes del despliegue de espacios de nombres en R 2.14.0,` Depends` era la única forma de "depender" de otro paquete. Ahora, a pesar del nombre, usted casi siempre debería usar `Imports`, no `Depends`. Aprenderá por qué, y cuando todavía seguir usando `Depends`, en [namespaces](http://r-pkgs.had.co.nz/namespace.html#namespace).
    
  También puede usar `Depends` para requerir una versión específica de R, p. Ej. `Depends: R (>= 3.0.1)`. Al igual que con los paquetes, es una buena idea ir a lo seguro y requerir una versión mayor o igual que la versión que está usando actualmente. `devtools::create()` hará esto por usted.
    
  En R 3.1.1 y anteriores también necesitará usar `Depends: methods` si usa S4. Este error se soluciona en R 3.2.0, por lo que los métodos pueden volver a `Imports` a donde pertenecen.
...
* `Title` es una descripción de una línea del paquete, y a menudo se muestra en listado de paquetes. Debe ser texto sin formato (sin marcado), en mayúscula como un título, y NO terminar en un punto. Que sea breve: las listas a menudo truncar el título a 65 caracteres.
...
Un buen título y descripción son importantes, especialmente si planea lanzar su paquete a CRAN porque aparecen en la página de descarga de CRAN de la siguiente manera:
...
Para identificar el autor del paquete y con quién contactar si algo sale mal, use el campo `Authors@R`. Este campo es inusual porque contiene código R ejecutable en lugar de texto sin formato. Aquí hay un ejemplo:
...
* Un código de tres letras que especifica el `role`. Hay cuatro roles importantes:
    ...
    * `cph`: titular de los derechos de autor. Esto se usa si el copyright está en manos de alguien que no sea el autor, generalmente una empresa (es decir, el empleador del autor).
    ...
    (La [lista completa de roles](http://www.loc.gov/marc/relators/relaterm.html) es extremadamente completo. Si su paquete tiene un leñador ("wdc"), letrista ("lyr") o diseñador de vestuario ("cst"), descanse cómodamente pues puede describir correctamente su papel en la creación de su paquete.)
...
* [MIT](https://tldrlegal.com/license/mit-license) (v. similar: a BSD 2 y 3 cláusulas de licencias). Este es una licencia simple y permisiva. Permite a las personas usar y distribuir libremente su código sujeto a una sola restricción: la licencia siempre debe ser distribuida con el código
...
* [CC0](https://tldrlegal.com/license/creative-commons-cc0-1.0-universal).
  ...
  Esta licencia es la más adecuada para paquetes de datos. Los datos, al menos en los EE. UU. no son susceptible de derechos de autor, por lo que realmente no está renunciando a nada. Esta licencia solo aclara este punto.
  
Si desea obtener más información sobre otras licencias comunes, Github's [choosealicense.com](http://choosealicense.com/licenses/) es un buen lugar para comenzar. Otro buen recurso es <https://tldrlegal.com/>, que explica las partes más importantes de cada licencia. Si usa una licencia distinta a las tres que sugiero, asegúrese de consultar la sección "Escribir extensiones en R" en [licencias](https://cran.r-project.org/doc/manuals/R-exts.html#The-DESCRIPTION-file).
...
Si desea liberar su paquete a CRAN, debe elegir una licencia estándar. De lo contrario, es difícil para CRAN determinar si es legal distribuir su paquete o no. Puede encontrar una lista completa de licencias que CRAN considera válidas en <https://svn.r-project.org/R/trunk/share/licenses/license.db>.
...
Formalmente, una versión de paquete R es una secuencia de al menos dos enteros separados por `.` o `-`. Por ejemplo, `1.0` y `0.9.1-10` son versiones válidas, pero `1` o `1.0-devel` no lo son. Puede analizar un número de versión con `numeric_version`.
...
Por ejemplo, un paquete puede tener una versión 1.9. R considera que este número de versión es igual a 1.9.0, menos que la versión 1.9.2, y todos estas son menores que la versión 1.10 (que es la versión "uno punto diez", no "uno punto uno cero"). R usa números de versión para determinar si se satisfacen las dependencias del paquete. Un paquete podría, por ejemplo, importar el paquete `devtools (> = 1.9.2)`, en cuyo caso la versión 1.9 o 1.9.0 no funcionaría.
...
Este consejo aquí está inspirado en parte por la [Versión semántica](http://semver.org) y por [X.Org](http://www.x.org/releases/X11R7.7/doc/xorg-docs/Versions.html) esquemas de control de versiones. Léelos si desea conocer más sobre los estándares de control de versiones utilizados por muchos proyectos de código abierto.

Volveremos a los números de versión en el contexto de la liberación de su paquete, [seleccionando un número de versión](http://r-pkgs.had.co.nz/release.html#release-version). Por ahora, solo recuerda que la primera versión de su paquete debería ser `0.0.0.9000`.

### Otros componentes

Varios otros campos se describen en otra parte del libro:
...
En realidad, hay muchos otros campos raros, si es que los usa alguna vez. Se puede encontrar una lista completa en la sección "The DESCRIPTION file" del [Manual de extensiones R](https://cran.r-project.org/doc/manuals/R-exts.html#The-DESCRIPTION-file). También puede crear sus propios campos para agregar metadatos adicionales. Las únicas restricciones son que no debe usar nombres existentes y que, si planea enviar a CRAN, los nombres que use deben ser palabras en inglés válidas (por lo que no se generará una NOTE de revisión ortográfica).
